...
class ListCommodityGradeRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->has('commodity_variety_id')) {
            $varId = $this->query('commodity_variety_id');
            $sanitized['commodity_variety_id'] = ($varId !== null && $varId !== '') ? (int) $varId : null;
        }
    ...
    public function rules(): array
    {
        return [
            ...
            'sort_by' => ['nullable', 'string', Rule::in(CommodityGrade::ALLOWED_SORT_COLUMNS)],
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            $varietyId = $this->query('commodity_variety_id');
            ...
            // 4. Variety + Commodity consistency
            if ($varietyId && $commodityId && ! $validator->errors()->has('commodity_variety_id') && ! $validator->errors()->has('commodity_id')) {
                $belongs = CommodityVariety::query()
                    ->where('id', (int) $varietyId)
                    ->where('commodity_id', (int) $commodityId)
                    ->whereNull('deleted_at')
                    ->exists();
            ...
            // 5. Variety + Subcategory consistency
            if ($varietyId && $subcategoryId && ! $validator->errors()->has('commodity_variety_id') && ! $validator->errors()->has('commodity_subcategory_id')) {
                $belongs = CommodityVariety::query()
                    ->where('id', (int) $varietyId)
                    ->where('commodity_subcategory_id', (int) $subcategoryId)
                    ->whereNull('deleted_at')
                    ->exists();
            ...
            // 6. Variety + Category consistency
            if ($varietyId && $categoryId && ! $validator->errors()->has('commodity_variety_id') && ! $validator->errors()->has('commodity_category_id')) {
                $belongs = CommodityVariety::query()
                    ->where('id', (int) $varietyId)
                    ->whereNull('deleted_at')
                    ->whereHas('commodity', function ($query) use ($categoryId) {
                ...
                if (! $belongs) {
                    $validator->errors()->add(
                        'commodity_variety_id',
                        'The selected commodity variety does not belong to the selected commodity category.'
                    );
                }
            }
        });
